#!/usr/bin/env python3
"""
mixnsfast.py -- mixns.py with LOSSLESS integer context keys (PyPy/Rust-friendly).

Identical model to mixns.py (non-stationarity + reach over the mixmax skeleton). The ONLY change:
the three dict-keyed hot paths (orders, sparse, word) use a single integer key instead of
(phase, bytes(...), ...) tuples -- a bijection, so output is bit-for-bit identical. Previous bytes are
carried in a rolling integer `htail` (no per-bit bytes allocation). The high-order tables already use
flat arrays; APM/match are unchanged.

Run `python mixnsfast.py flip` for the causality self-test, or under pypy3 for the speedup.
"""
...
def run(bits, lr=0.0085, lr_final=0.01, delta=0.18, record=False,
    ...
    htail = 0                                              # rolling int of last MAXB completed bytes
    ...
    for i in range(n):
        ...
        for k in range(NM):
            ...
            run_ = ((htail & MASKB[L]) << phase) | cur          # prev bytes ++ partial byte
            key = (((1 << (8 * L + phase)) | run_) << 3) | phase  # lossless: == (phase, partial, prev)
            c = tables[k].get(key)
            if c is None:
        ...
        sk = ((((((1 << phase) | cur) << 8) | b2) << 8 | b3) << 3) | phase   # lossless sparse key
        c = sparse.get(sk)
        if c is None:
            c = [0, 0]; sparse[sk] = c
        sp = c; sts[I_SP] = stretch((c[1] + delta) / (c[0] + c[1] + 2 * delta))
        wk = (word_hash << 12) | ((((1 << phase) | cur) << 3) | phase)        # lossless word key
        c = wordt.get(wk)
        if c is None:
            c = [0, 0]; wordt[wk] = c
...
def main():
    ...
    print(f"  mixnsfast  whole-stream = {whole:.6f}   last-20% = {tail:.6f}  bits/bit")
