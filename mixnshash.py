#!/usr/bin/env python3
"""
mixnshash.py -- mixnsfast.py with the orders/sparse/word tables as FIXED-SIZE HASHED arrays.

Same model as mixns/mixnsfast, but the growing Python dicts for the byte-orders, the sparse model and
the word model are replaced by fixed open-addressing-free hashed count arrays (the high orders already
worked this way). This BOUNDS memory regardless of corpus size -- the standard real-compressor design,
and exactly the Rust/C++ data structure. Cost: hash collisions (different contexts can share a slot),
a small *measured* quality tradeoff vs the exact dict version.

The lossless integer context key (from mixnsfast) is hashed into the table:  slot = (key*MULT & mask)<<1.

Run `python mixnshash.py flip` (causality), or under pypy3 for scale.  obits sets table size (2^obits slots).
"""
...
CLIMIT = 255
MAXB = max(ORDERS)
MASKB = [(1 << (8 * L)) - 1 for L in range(MAXB + 1)]
OBITS = 22                                              # 4M slots per order/sparse/word table
MULT = 2654435761
...
def run(bits, lr=0.0085, lr_final=0.01, delta=0.18, record=False,
        recency=True, adaptive_lr=True, reach=True, climit=CLIMIT, hbits=HBITS, obits=OBITS,
    ...
    hmask = (1 << hbits) - 1
    omask = (1 << obits) - 1
    I_H = NM; I_SP = NM + NH; I_WD = NM + NH + 1; I_MT = NM + NH + 2; NIN = NM + NH + 3
    # fixed-size hashed count tables (2 cells per slot): orders, sparse, word
    otables = [array('I', bytes(4 * 2 * (1 << obits))) for _ in range(NM)]
    sp_t = array('I', bytes(4 * 2 * (1 << obits)))
    wd_t = array('I', bytes(4 * 2 * (1 << obits)))
    # per-slot 8-bit checksum tags: on a tag mismatch we EVICT (reset) the slot rather than merge
    # unrelated contexts -- the standard PAQ/zpaq trick to bound memory with little quality loss.
    otag = [array('B', bytes(1 << obits)) for _ in range(NM)]
    sp_tag = array('B', bytes(1 << obits))
    wd_tag = array('B', bytes(1 << obits))
    ...
    gmix_g = [0.0] * (NIN + 1)
    final_w = [0.3, 0.3, 0.0]; final_g = [0.0, 0.0, 0.0]
    apm1 = APM(256 * 8, rate=0.007); apm2 = APM(1024, rate=0.005)
    match = MatchModel()
    hist = bytearray(); cur = 0; prev_byte = 0; prev2 = 0; word_hash = 0; byte_pos = 0; htail = 0
    sts = [0.0] * (NIN + 1)
    oslots = [0] * NM; hidxs = [0] * NH; hbase = [0] * NH
    ...
    for i in range(n):
        ...
        for k in range(NM):
            ...
            run_ = ((htail & MASKB[L]) << phase) | cur
            key = (((1 << (8 * L + phase)) | run_) << 3) | phase
            h = key * MULT
            ti = h & omask; slot = ti << 1
            ot = otables[k]; tg = otag[k]; want = (h >> obits) & 0xFF
            if tg[ti] != want:
                tg[ti] = want; ot[slot] = 0; ot[slot + 1] = 0
            oslots[k] = slot
            n0 = ot[slot]; n1 = ot[slot + 1]
            sts[k] = stretch((n1 + delta) / (n0 + n1 + 2 * delta))
        ...
        b2 = hist[byte_pos - 2] if byte_pos >= 2 else 0
        b3 = hist[byte_pos - 3] if byte_pos >= 3 else 0
        sk = ((((((1 << phase) | cur) << 8) | b2) << 8 | b3) << 3) | phase
        h = sk * MULT
        sp_ti = h & omask; sp_slot = sp_ti << 1; want = (h >> obits) & 0xFF
        if sp_tag[sp_ti] != want:
            sp_tag[sp_ti] = want; sp_t[sp_slot] = 0; sp_t[sp_slot + 1] = 0
        n0 = sp_t[sp_slot]; n1 = sp_t[sp_slot + 1]
        sts[I_SP] = stretch((n1 + delta) / (n0 + n1 + 2 * delta))
        wk = (word_hash << 12) | ((((1 << phase) | cur) << 3) | phase)
        h = wk * MULT
        wd_ti = h & omask; wd_slot = wd_ti << 1; want = (h >> obits) & 0xFF
        if wd_tag[wd_ti] != want:
            wd_tag[wd_ti] = want; wd_t[wd_slot] = 0; wd_t[wd_slot + 1] = 0
        n0 = wd_t[wd_slot]; n1 = wd_t[wd_slot + 1]
        sts[I_WD] = stretch((n1 + delta) / (n0 + n1 + 2 * delta))
        ...
        if recency:
            for k in range(NM):
                ot = otables[k]; slot = oslots[k]
                ot[slot + y] += 1
                if ot[slot + y] >= climit:
                    ot[slot] = (ot[slot] + 1) >> 1; ot[slot + 1] = (ot[slot + 1] + 1) >> 1
            sp_t[sp_slot + y] += 1
            if sp_t[sp_slot + y] >= climit:
                sp_t[sp_slot] = (sp_t[sp_slot] + 1) >> 1; sp_t[sp_slot + 1] = (sp_t[sp_slot + 1] + 1) >> 1
            wd_t[wd_slot + y] += 1
            if wd_t[wd_slot + y] >= climit:
                wd_t[wd_slot] = (wd_t[wd_slot] + 1) >> 1; wd_t[wd_slot + 1] = (wd_t[wd_slot + 1] + 1) >> 1
            for hk_i in range(NH):
            ...
            for k in range(NM):
                ot = otables[k]; ot[oslots[k] + y] += 1
            sp_t[sp_slot + y] += 1; wd_t[wd_slot + y] += 1
            for hk_i in range(NH):
...
def main():
    ...
    print(f"  mixnshash  whole-stream = {whole:.6f}   last-20% = {tail:.6f}  bits/bit")
